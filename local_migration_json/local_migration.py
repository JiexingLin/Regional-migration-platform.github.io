...
class JapaneseMigrationAgent:
    ...
    async def search_with_serpapi(self, query: str) -> tuple:
        """
        使用SerpAPI进行搜索并返回完整结果和前两条结果的URL
        """
        try:
            # # 获取SerpAPIWrapper内部的客户端
            # serpapi_client = self.search.search_engine
            ...
            # 从organic_results中提取URL
            urls = []
            if 'organic_results' in results:
                urls = [result['link'] for result in results.get('organic_results', [])[:2]]
            
            # 为了兼容性，仍然使用search.run获取文本结果
            full_result = self.search.run(query)
            
            return full_result, urls
        except Exception as e:
